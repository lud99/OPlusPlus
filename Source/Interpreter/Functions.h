...
namespace Ö
{
	...
	{
		#define ARGS std::vector<Value> args
		typedef Value(*BuiltInFunctionCallable)(ARGS);

		struct Prototype
		{
			uint16_t id;
			std::string name;
			BuiltInFunctionCallable functionPointer;

			ValueType returnType;

			Prototype() {};
			Prototype(uint16_t _id, std::string _name, BuiltInFunctionCallable _functionPointer, ValueType _returnType) :
				id(_id), name(_name), functionPointer(_functionPointer), returnType(_returnType) {};
